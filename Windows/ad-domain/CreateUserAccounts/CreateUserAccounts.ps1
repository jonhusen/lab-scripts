...
$dirpath = $pwd.path
$orgUnit = "CN=Users"
$dummyPassword = ConvertTo-SecureString -AsPlainText "Pa55w0rd" -Force
$counter = 0
...
$ImportFile | foreach {
	...
	if ($_.Manager -eq "") {
		New-ADUser -SamAccountName $_.SamAccountName -Name $_.Name -Surname $_.Sn -GivenName $_.GivenName -Path "$orgUnit,$domain" -AccountPassword $dummyPassword -Enabled $true -title $_.title -officePhone $_.officePhone -department $_.department -emailaddress $_.mail -EmployeeID $_.employeeid -PasswordNeverExpires $true
	} else {
...
#Set-AdUser danj -EmployeeID "12345"